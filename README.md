...
│   └── evaluation.ipynb               # Loads a saved checkpoint and computes threshold sweep + crater-level metrics
├── training/                          # Separate GPU pipeline for full-tile DEM-only/dual-channel training, run outside the notebooks
│   ├── convert_to_memmap.py           # One-time .npz -> memory-mapped .npy conversion for GPU streaming
│   ├── gpu_data.py                    # Standalone patch loading for the GPU run
│   ├── train_gpu.py                   # Trains one model (dem or dual) on the full tile
│   ├── evaluate.py                    # Pixel-level metrics, PR curve, confusion matrix, overlay figures
│   ├── postprocess.py                 # Hough-transform circle extraction and crater-level matching
│   └── run_all.py                     # Runs evaluate.py + postprocess.py for one trained model
├── environment.yml                    # Conda environment
└── README.md                          # This file
```

`filtered_labels.csv` / `filtered_labels_alltiles.csv` are generated by the merge notebooks (`data_merge.ipynb` / `data_merge_alltiles.ipynb`) and are gitignored, not committed — the full-band version alone is ~180MB, over GitHub's 100MB file limit, and both regenerate from the Robbins catalogue in seconds with no raster downloads. Run the relevant merge notebook once locally before running the matching pre-processing notebook. `3_pre_processing/make_labels.py` regenerates the single-tile `filtered_labels.csv` the same way without opening the notebook.
